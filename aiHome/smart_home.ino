...
// WIFI / BRIDGE
// =====================================================================
const char *BRIDGE_HOST = "192.168.1.103";
...
void servoSet(int angle, int durationMs) {
  ...
  while (millis() - start < (unsigned long)durationMs) {
    servoWrite(angle);
    vTaskDelay(
        pdMS_TO_TICKS(20)); // Thay busy-wait = yield RTOS, chống rớt mạng
  }
}
...
struct State {
  ...
  float dhtTemp = 0.0;
  float dhtHum = 0.0;
  // Trạng thái thiết bị — dùng cho get_devices_status (digitalRead OUTPUT sai
  // trên R4)
  bool fanOn = false;
...
size_t executeCommand(const char *tool, const char *pollJson, char *out,
  ...
  if (!strcmp(tool, "get_environment")) {
    ...
    snprintf(out, outCap,
             "{\"device\":\"dht11\",\"temperature\":%.1f,\"humidity\":%.1f,"
             "\"success\":true}",
             t, h);
    return strlen(out);
  }
  if (!strcmp(tool, "get_sensors_status")) {
    ...
    if (rainP >= 70)
      ...
      rainStatus = "am uot nhe";
    else
      rainStatus = "troi kho rao";

    const char *fireStatus =
        fire ? "phat hien lua chay - nguy hiem" : "an toan khong co lua";
    const char *alarmStatus = fireAr ? "coi bao dong dang ket" : "coi im lang";
    const char *clothStatus = rainRetract ? "gian phoi da thu vao (do mua)"
                                          : "gian phoi dang mo ra (troi kho)";
    ...
    snprintf(out, outCap,
             ...
             "\"success\":true}",
             rainP, rainStatus, fire ? "true" : "false", fireStatus,
  ...
  if (!strcmp(tool, "get_devices_status")) {
    ...
    snprintf(
        ...
        "\"success\":true}",
        fan ? "on" : "off", lt1 ? "on" : "off", lt2 ? "on" : "off",
        lt3 ? "on" : "off", ltY ? "on" : "off",
        fan ? "quat dang chay" : "quat tat", lt1 ? "sang" : "tat",
        lt2 ? "sang" : "tat", lt3 ? "sang" : "tat", ltY ? "sang" : "tat");
    return strlen(out);
  }
